...
class BilanBioPermissions(permissions.BasePermission):
    """
    ...
    def has_permission(self, request, view):
        ...
        elif request.method in ['PATCH', 'OPTIONS']:
...
class BilanRadioPermissions(permissions.BasePermission):
    """
    Custom permission to allow:
    - GET requests for users in the 'medecin', 'laboratorien', or 'patient' groups.
    - POST requests only for users in the 'medecin' group.
    ...
    def has_permission(self, request, view):
        ...
        # For POST method, allow only 'medecin' group
            ...
            return request.user.groups.filter(name= 'radiologue').exists()
